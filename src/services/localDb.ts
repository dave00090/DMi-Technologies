...
export const localDb = {
  ...
  addProduct: async (product: Omit<Product, 'id'>) => {
    const all = getLocal<Product[]>(STORAGE_KEYS.PRODUCTS, []);
    const newProduct = { ...product, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Product;
    await setLocal(STORAGE_KEYS.PRODUCTS, [...all, newProduct]);
    return newProduct;
  },
  updateProduct: async (id: string, updates: Partial<Product>) => {
    const all = getLocal<Product[]>(STORAGE_KEYS.PRODUCTS, []);
    const updated = all.map(p => p.id === id ? { ...p, ...updates, synced: false, lastUpdated: new Date().toISOString() } : p);
  ...
  addCustomer: async (customer: Omit<Customer, 'id'>) => {
    const all = getLocal<Customer[]>(STORAGE_KEYS.CUSTOMERS, []);
    const newCustomer = { ...customer, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Customer;
    await setLocal(STORAGE_KEYS.CUSTOMERS, [...all, newCustomer]);
    return newCustomer;
  },
  updateCustomer: async (id: string, updates: Partial<Customer>) => {
    const all = getLocal<Customer[]>(STORAGE_KEYS.CUSTOMERS, []);
    const updated = all.map(c => c.id === id ? { ...c, ...updates, synced: false, lastUpdated: new Date().toISOString() } : c);
  ...
  addAlert: async (alert: Omit<Alert, 'id'>) => {
    const all = getLocal<Alert[]>(STORAGE_KEYS.ALERTS, []);
    const newAlert = { ...alert, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Alert;
    await setLocal(STORAGE_KEYS.ALERTS, [newAlert, ...all]);
    return newAlert;
  },
  updateAlert: async (id: string, updates: Partial<Alert>) => {
    const all = getLocal<Alert[]>(STORAGE_KEYS.ALERTS, []);
    const updated = all.map(a => a.id === id ? { ...a, ...updates, synced: false, lastUpdated: new Date().toISOString() } : a);
  ...
  addBusiness: async (business: Omit<BusinessProfile, 'id'>) => {
    ...
    const newBusiness = { ...business, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() };
  ...
  updateBusiness: async (id: string, updates: Partial<BusinessProfile>) => {
    const businesses = localDb.getBusinesses();
    const updated = businesses.map(b => b.id === id ? { ...b, ...updates, synced: false, lastUpdated: new Date().toISOString() } : b);
  ...
  addShop: async (shop: Omit<Shop, 'id'>) => {
    const all = getLocal<Shop[]>(STORAGE_KEYS.SHOPS, []);
    const newShop = { ...shop, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() };
    await setLocal(STORAGE_KEYS.SHOPS, [...all, newShop]);
    return newShop;
  },
  updateShop: async (id: string, updates: Partial<Shop>) => {
    const all = getLocal<Shop[]>(STORAGE_KEYS.SHOPS, []);
    const updated = all.map(s => s.id === id ? { ...s, ...updates, synced: false, lastUpdated: new Date().toISOString() } : s);
  ...
  addExpense: async (expense: Omit<Expense, 'id'>) => {
    const all = getLocal<Expense[]>(STORAGE_KEYS.EXPENSES, []);
    const newExpense = { ...expense, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Expense;
  ...
  addSupplier: async (supplier: Omit<Supplier, 'id'>) => {
    const all = getLocal<Supplier[]>(STORAGE_KEYS.SUPPLIERS, []);
    const newSupplier = { ...supplier, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Supplier;
  ...
  addEmployee: async (employee: Omit<Employee, 'id'>) => {
    const all = getLocal<Employee[]>(STORAGE_KEYS.EMPLOYEES, []);
    const newEmployee = { ...employee, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Employee;
    await setLocal(STORAGE_KEYS.EMPLOYEES, [...all, newEmployee]);
    return newEmployee;
  },
  updateEmployee: async (id: string, updates: Partial<Employee>) => {
    const all = getLocal<Employee[]>(STORAGE_KEYS.EMPLOYEES, []);
    const updated = all.map(e => e.id === id ? { ...e, ...updates, synced: false, lastUpdated: new Date().toISOString() } : e);
  ...
  addAttendance: async (attendance: Omit<Attendance, 'id'>) => {
    const all = getLocal<Attendance[]>(STORAGE_KEYS.ATTENDANCE, []);
    const newAttendance = { ...attendance, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Attendance;
  ...
  addPayroll: async (payroll: Omit<Payroll, 'id'>) => {
    const all = getLocal<Payroll[]>(STORAGE_KEYS.PAYROLL, []);
    const newPayroll = { ...payroll, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Payroll;
  ...
  addDebt: async (debt: Omit<Debt, 'id'>) => {
    const all = getLocal<Debt[]>(STORAGE_KEYS.DEBTS, []);
    const newDebt = { ...debt, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as Debt;
    await setLocal(STORAGE_KEYS.DEBTS, [newDebt, ...all]);
    return newDebt;
  },
  updateDebt: async (id: string, updates: Partial<Debt>) => {
    const all = getLocal<Debt[]>(STORAGE_KEYS.DEBTS, []);
    const updated = all.map(d => d.id === id ? { ...d, ...updates, synced: false, lastUpdated: new Date().toISOString() } : d);
  ...
  addLedgerEntry: async (entry: Omit<LedgerEntry, 'id'>) => {
    const all = getLocal<LedgerEntry[]>(STORAGE_KEYS.LEDGER, []);
    const newEntry = { ...entry, id: crypto.randomUUID(), synced: false, lastUpdated: new Date().toISOString() } as LedgerEntry;
