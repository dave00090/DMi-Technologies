...
-- It handles missing columns (like entity_id, business_id) gracefully.
...
-- 3. SCHEMA HARDENING (Adding missing columns to existing tables via DO block)
DO $$
BEGIN
    ...
    ELSE
        -- Ensure columns exist in ledger_entries if table exists but is outdated
        ...
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='ledger_entries' AND column_name='entity_id') THEN
            -- If data exists, we might need a default or allow null first, then set NOT NULL later
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='sales') THEN
        CREATE TABLE sales (
            ...
            change DECIMAL,
            client_name TEXT -- Added for Master Admin tracking
        );
    ELSE
    ...
    -- LICENSES (Wait, Licenses table is usually the main one, ensure it's correct)
    ...
    -- OTHER TABLES - ENSURE BUSINESS_ID EXISTS
    -- Products
